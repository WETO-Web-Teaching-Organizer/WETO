...
<form action="<s:url action="editGradeDiscussionMessage"/>" method="post" onsubmit="return pollLogin()">
  ...
  <s:a href="%{discussGradeUrl}" cssClass="linkButton">
    <s:text name="general.header.cancel" />
  </s:a>
  <s:if test="navigator.teacher">
    <s:url action="editGradeDiscussionMessage" var="deleteGradeDiscussionMessageUrl">
      <s:param name="taskId" value="%{taskId}" />
      <s:param name="tabId" value="%{tabId}" />
      <s:param name="dbId" value="%{dbId}" />
      <s:param name="gradeId" value="%{gradeId}" />
    ...
    <s:a href="%{deleteGradeDiscussionMessageUrl}" cssClass="btn btn-danger" cssStyle="float: right">
