...
/**
 * Generated database access class for table Admin.
 * Table for storing admins
 */
public class DbAdmin extends SqlAssignableObject implements Cloneable {
    ...
    public boolean equals( Object obj ) {
        ...
        if( !(obj instanceof DbAdmin) ) return false;
        DbAdmin dbObj = (DbAdmin)obj;
    ...
    /**
     * Inserts this object to the database table Admin.
    ...
    public void insert(Connection con) throws SQLException, ObjectNotValidException {
        ...
        String prepareString = "insert into Admin (userId, timeStamp) values (?, ?);";
    ...
    /**
     * Updates the row of this object in the database table Admin.
    ...
    public void update(Connection con) throws SQLException, ObjectNotValidException, NoSuchItemException {
        if( !userIdData.isValid() ) throw new ObjectNotValidException("userId");
        if( !timeStampData.isValid() ) throw new ObjectNotValidException("timeStamp");
        String prepareString = "update Admin set userId = ?, timeStamp = ? where userId = ?";
        PreparedStatement ps = con.prepareStatement(prepareString);
        ps.setObject(1, userIdData.jdbcGetValue());
        ps.setObject(2, timeStampData.jdbcGetValue());
    ...
    /**
     * Deletes the row of this object from the database table Admin.
     * The row is identified by the primary key attribute(s).
     * @param con Open and active connection to the database.
     * @throws SQLException if the JDBC operation fails.
    ...
    public void delete(Connection con) throws SQLException, TooManyItemsException, NoSuchItemException {
        String prepareString = "delete from Admin where userId = ?";
        PreparedStatement ps = con.prepareStatement(prepareString);
    ...
    /**
     * Selects the row of this object from the database table Admin and updates the attributes accordingly.
     * The row is identified by the primary key attribute(s).
     * @param con Open and active connection to the database.
     * @throws SQLException if the JDBC operation fails.
     * @throws InvalidValueException if the attributes are invalid.
    ...
    public void select(Connection con) throws SQLException, InvalidValueException, NoSuchItemException {
        String prepareString = "select * from Admin where userId = ?";
        PreparedStatement ps = con.prepareStatement(prepareString);
        ps.setObject(1, userIdData.jdbcGetValue());
    ...
    /**
     * Constructs and returns a selection iterator for rows in database table Admin.
     * 
     * @param con Open and active connection to the database.
     * @param whereClause Optional where clause. If null is given, all the rows are selected.
     * 
     * @return Newly constructed iterator that returns objects of type Admin. The iterator is closed when hasNext() returns false or the iterator is finalized.
     * 
     * @throws SQLException if the JDBC operation fails.
     * 
    ...
    public static Iterator selectionIterator( Connection con, String whereClause ) throws SQLException {
        ...
        if( whereClause.equals("") )
            prepareString = "select * from Admin";
        else
            prepareString = "select * from Admin where " + whereClause;
        PreparedStatement ps = con.prepareStatement(prepareString);
        return new SqlSelectionIterator( ps, Admin.class );
    }
}
