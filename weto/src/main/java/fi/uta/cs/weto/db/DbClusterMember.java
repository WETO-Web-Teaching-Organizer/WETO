...
/**
 * Generated database access class for table ClusterMember.
 * 
 */
public class DbClusterMember extends SqlAssignableObject implements Cloneable {
    private SqlInteger clusterIdData;
    private SqlInteger clusterIdKC;
    ...
    public DbClusterMember() {
        super();
        clusterIdData = new SqlInteger();
        clusterIdKC = new SqlInteger();
        userIdData = new SqlInteger();
        userIdKC = new SqlInteger();
        timeStampData = new SqlInteger();
        clusterIdData.setPrime(true);
    ...
    public void setFromResultSet(ResultSet resultSet, int baseIndex) throws SQLException, InvalidValueException {
        clusterIdData.jdbcSetValue( (java.lang.Integer) resultSet.getObject(baseIndex+1) );
        clusterIdKC.jdbcSetValue(clusterIdData.jdbcGetValue());
    ...
    public String toString() {
        ...
        sb.append("ClusterMember\n");
        sb.append("clusterId:" +  clusterIdData.toString() + "\n");
    ...
    public boolean equals( Object obj ) {
        ...
        if( !(obj instanceof DbClusterMember) ) return false;
        DbClusterMember dbObj = (DbClusterMember)obj;
        if( !clusterIdData.equals( dbObj.clusterIdData ) ) return false;
    ...
    /**
     * Gets the raw data object for clusterId attribute.
     * @return Data object as SqlInteger.
     */
    public SqlInteger getClusterIdData() {
        return clusterIdData;
    }
    ...
    /**
     * Inserts this object to the database table ClusterMember.
    ...
    public void insert(Connection con) throws SQLException, ObjectNotValidException {
        if( !clusterIdData.isValid() ) throw new ObjectNotValidException("clusterId");
        ...
        String prepareString = "insert into ClusterMember (clusterId, userId, timeStamp) values (?, ?, ?);";
        PreparedStatement ps = con.prepareStatement(prepareString);
        ps.setObject(1, clusterIdData.jdbcGetValue());
    ...
    /**
     * Updates the row of this object in the database table ClusterMember.
    ...
    public void update(Connection con) throws SQLException, ObjectNotValidException, NoSuchItemException {
        ...
        if( !userIdData.isValid() ) throw new ObjectNotValidException("userId");
        if( !timeStampData.isValid() ) throw new ObjectNotValidException("timeStamp");
        String prepareString = "update ClusterMember set clusterId = ?, userId = ?, timeStamp = ? where clusterId = ? AND userId = ?";
        PreparedStatement ps = con.prepareStatement(prepareString);
        ps.setObject(1, clusterIdData.jdbcGetValue());
        ps.setObject(2, userIdData.jdbcGetValue());
        ps.setObject(3, timeStampData.jdbcGetValue());
        ps.setObject(4, clusterIdKC.jdbcGetValue());
    ...
    /**
     * Deletes the row of this object from the database table ClusterMember.
     * The row is identified by the primary key attribute(s).
     * @param con Open and active connection to the database.
     * @throws SQLException if the JDBC operation fails.
    ...
    public void delete(Connection con) throws SQLException, TooManyItemsException, NoSuchItemException {
        String prepareString = "delete from ClusterMember where clusterId = ? AND userId = ?";
        PreparedStatement ps = con.prepareStatement(prepareString);
        ps.setObject(1, clusterIdKC.jdbcGetValue());
        ps.setObject(2, userIdKC.jdbcGetValue());
        int rows = ps.executeUpdate();
        ps.close();
    ...
    /**
     * Selects the row of this object from the database table ClusterMember and updates the attributes accordingly.
     * The row is identified by the primary key attribute(s).
     * @param con Open and active connection to the database.
     * @throws SQLException if the JDBC operation fails.
     * @throws InvalidValueException if the attributes are invalid.
    ...
    public void select(Connection con) throws SQLException, InvalidValueException, NoSuchItemException {
        String prepareString = "select * from ClusterMember where clusterId = ? AND userId = ?";
        PreparedStatement ps = con.prepareStatement(prepareString);
        ps.setObject(1, clusterIdData.jdbcGetValue());
        ps.setObject(2, userIdData.jdbcGetValue());
    ...
    /**
     * Constructs and returns a selection iterator for rows in database table ClusterMember.
     * 
     * @param con Open and active connection to the database.
     * @param whereClause Optional where clause. If null is given, all the rows are selected.
     * 
     * @return Newly constructed iterator that returns objects of type ClusterMember. The iterator is closed when hasNext() returns false or the iterator is finalized.
     * 
     * @throws SQLException if the JDBC operation fails.
     * 
    ...
    public static Iterator selectionIterator( Connection con, String whereClause ) throws SQLException {
        ...
        if( whereClause.equals("") )
            prepareString = "select * from ClusterMember";
        else
            prepareString = "select * from ClusterMember where " + whereClause;
        PreparedStatement ps = con.prepareStatement(prepareString);
        return new SqlSelectionIterator( ps, ClusterMember.class );
    }
}
